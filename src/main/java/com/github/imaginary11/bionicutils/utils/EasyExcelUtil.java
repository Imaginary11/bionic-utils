...
import com.github.imaginary11.bionicutils.model.EasyExcelModel;
import com.github.imaginary11.bionicutils.model.EasyExcelStringModel;
...
public class EasyExcelUtil {
     ...
     * @param excelTypeEnum Excel的格式(XLS或XLSX)
     */
    public static void writeMultySheetExcelWithStringList(OutputStream outputStream, List<EasyExcelStringModel> easyExcelStringModels, ExcelTypeEnum excelTypeEnum) {
        //这里指定不需要表头，因为String通常表头已被包含在data里
        ExcelWriter writer = new ExcelWriter(outputStream, excelTypeEnum, false);
        ...
        if (easyExcelStringModels != null && easyExcelStringModels.size() > 0) {
            for (EasyExcelStringModel easyExcelStringModel : easyExcelStringModels) {
                Sheet sheet = easyExcelStringModel.getSheet();
                List<List<String>> data = easyExcelStringModel.getData();
    ...
    /**
     * 使用 模型 来写入Excel,多个sheet
     *
     * @param outputStream
     * @param easyExcelModels
     * @param excelTypeEnum
     */
    public static void writeMultySheetExcelWithModel(OutputStream outputStream, List<EasyExcelModel> easyExcelModels, ExcelTypeEnum excelTypeEnum) {
        ExcelWriter writer = new ExcelWriter(outputStream, excelTypeEnum, true);
        ...
        if (easyExcelModels != null && easyExcelModels.size() > 0) {
            for (EasyExcelModel easyExcelModel : easyExcelModels) {
                Sheet sheet = easyExcelModel.getSheet();
                List data = easyExcelModel.getData();
